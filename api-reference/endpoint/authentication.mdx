...
description: 'Learn how to authenticate your API requests and retrieve your API keys'
---

To interact with the CreptaPay API, you must authenticate your requests using API keys. The API supports two environments: Sandbox (for testing) and Production (for live transactions).
...
4. You will find two sections:
   - **Public Keys**: These keys begin with `pk_` and are safe for client-side environments.
   - **Secret Keys**: These keys begin with `sk_` and must be kept secure on your server.
